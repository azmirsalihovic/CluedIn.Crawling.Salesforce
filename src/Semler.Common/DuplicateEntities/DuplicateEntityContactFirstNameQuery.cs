// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DuplicateEntityContactFirstNameQuery.cs" company="Clued In">
//   Copyright (c) 2019 Clued In. All rights reserved.
// </copyright>
// <summary>
//   Implements the duplicate entity First Name query class.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
{
    public class DuplicateEntityContactFirstNameQuery : IOnlineDuplicateEntityQuery
    {
        public string Name { get; } = "First Name";

        public string DisplayName { get; } = "Duplicate Entity First Names";
        ...
        public async Task<IEnumerable<IDuplicateEntityQueryResultSet>> GetPotentialDuplicatesAsync(ExecutionContext context, EntityType entityType = null)
        {
            ...
            foreach (var type in entityTypes)
            {
                var query = new ParsedQuery();
                query.Query = "*";
                query.Fields = new List<string>() { "properties.semler.contact.FirstName" };
                query.Cursor = PagingCursor.Default;
                query.Aggregations = new List<AggregationQuery>() { new TermAggregationQuery("properties.semler.contact.FirstName", 150) };
                query.RankingSettings = ParsedQuery.DefaultRanking;
                query.IncludeUnstructuredData = !ConfigurationManagerEx.AppSettings.GetFlag("Feature.Filters.ShadowEntities", true);
                query.OptionalFields = new List<string>();
                query.SearchSpecificEntityTypesByName = new List<string>();
            ...
            query.Filters = ParsedFilteringQuery.Parse(context, query, null, new[]
                                                                             {
                                                                                 ...
                                                                                 new FilterQuery()
                                                                                 {
                                                                                     FieldName       = "properties.semler.contact.FirstName",
                                                                                     AggregationName = "properties.semler.contact.FirstName",
                                                                                     Operator        = DefaultSearchOperator.And,
